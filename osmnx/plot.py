...
def plot_graph_routes(G, routes, targets, bbox=None, fig_height=6, fig_width=None,
                      margin=0.02, bgcolor='w', axis_off=True, show=True,
                      save=False, close=True, file_format='png', filename='temp',
                      dpi=300, annotate=False, node_color='#999999',
                      node_size=15, node_alpha=1, node_edgecolor='none',
                      ...
                      route_alpha=0.5, orig_dest_node_alpha=0.5,
                      orig_dest_node_size=100, orig_dest_node_color='r',
                      orig_dest_point_color='b', orig_color='b', dest_color = 'r'):
    """
    ...
    dest_points_lats = []
    dest_points_lons = []
    old_min = float((min(targets.items(), key = lambda x : x[1]))[1])
    old_max = float((max(targets.items(), key = lambda x : x[1]))[1])
    ...
    if orig_dest_points is None:
        ...
        for route in routes:
            origin_node = route[0]
            destination_node = route[-1]
            orig_points_lats.append(G.nodes[origin_node]['y'])
            dest_points_lats.append(G.nodes[destination_node]['y'])
            orig_points_lons.append(G.nodes[origin_node]['x'])
            dest_points_lons.append(G.nodes[destination_node]['x'])
            new_value = (((targets[destination_node] - old_min)*new_range)/old_range) + 100
            sizes.append(new_value)

    else:
        # otherwise, use the passed points as origin/destination points
        for point in orig_dest_points:
            orig_points_lats.append(point[0])
            orig_points_lons.append(point[1])
        orig_dest_node_color = orig_dest_point_color

    # scatter the origin and destination points
    ax.scatter(orig_points_lons, orig_points_lats, s=orig_dest_node_size,
               c=orig_color, alpha=orig_dest_node_alpha, edgecolor=node_edgecolor, zorder=4)

    ax.scatter(dest_points_lons, dest_points_lats, s=sizes,
               c=dest_color, alpha=orig_dest_node_alpha, edgecolor=node_edgecolor, zorder=4)

    # plot the routes lines
    lines = []
    for route in routes:
        percorso = []
        edge_nodes = list(zip(route, route[1:]))
        for u, v in edge_nodes:
            # if it doesn't have a geometry attribute, the edge is a straight
            # line from node to node
            x1 = G.nodes[u]['x']
            ...
            y2 = G.nodes[v]['y']
            line = [(x1, y1), (x2, y2)]
            percorso.append((x1, y1))
            percorso.append((x2, y2))
        lines.append(percorso)

    # add the lines to the axis as a linecollection
    colors = np.asarray(cm.rainbow(np.linspace(0, 1, len(lines))))
    lc = LineCollection(lines, colors=colors, linewidths=route_linewidth, alpha=route_alpha, zorder=3)
    ax.add_collection(lc)
